...
export default function VerifyPendingPage() {
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-4">
      <div className="bg-white rounded-[12px] border border-line p-10 max-w-[480px] w-full text-center">
        ...
        <h1 className="text-navy text-[24px] font-[600] mb-3">Sprawdź skrzynkę email</h1>
        <p className="text-secondary text-[15px] leading-[160%] mb-2">
          Wysłaliśmy link aktywacyjny na adres:
        </p>
        {email && (
          <p className="text-ink font-[600] text-[15px] mb-6">{email}</p>
        )}
        <p className="text-secondary text-[14px] leading-[160%] mb-8">
          Kliknij link w emailu, aby aktywować konto. Link wygaśnie po 24 godzinach.
        </p>

        <div className="border-t border-line pt-6">
          <p className="text-tertiary text-[13px] mb-3">Nie dostałeś emaila?</p>
          <button
            onClick={handleResend}
            disabled={resending || !email}
            className="text-navy font-[500] text-[14px] hover:underline disabled:opacity-50 disabled:no-underline"
          >
            {resending ? "Wysyłanie..." : "Wyślij ponownie"}
          </button>
          {resendMessage && (
            <p className="text-success text-[13px] mt-3">{resendMessage}</p>
          )}
          {resendError && (
            <p className="text-danger text-[13px] mt-3">{resendError}</p>
          )}
        </div>
      </div>
    </main>
